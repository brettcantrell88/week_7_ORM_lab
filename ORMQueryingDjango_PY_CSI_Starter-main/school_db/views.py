...
def problem_two(request):
    instructors = Instructor.objects.filter(hire_date__gt = '2010-01-01').order_by('-hire_date')
...
def problem_three(request):
    ...
    results = Course.objects.filter(instructor_id=2)
    for item in results:
      print(f'Course Name: {item.name}')
    return complete(request)
...
def problem_four(request):
    ...
    print(f'There are {course_count} courses') 
    print(f'There are {instructor_count} instructors')
    return complete(request)
...
def problem_five(request):
    student = Student.objects.create(first_name = "Brett", last_name = "Cantrell", year = 1988, gpa = 3.5)
    print(f'The new student is: {student.id}, {student.first_name}, {student.last_name}, {student.year} {student.gpa}')
    return complete(request)
...
def problem_six(request):
    Student.objects.filter(id = 15).update(gpa = 1.5)
    student = Student.objects.get(pk=15)
    print(f'New Student GPA: {student.id}, {student.first_name}, {student.last_name}, {student.gpa}')
...
def problem_seven(request):
    ...
    # Make sure to set this equal to the primary key of the row you just created!
    student_id = 15
    Student.objects.filter(id = 15).delete()
    try:
